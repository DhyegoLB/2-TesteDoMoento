...
namespace TestedoMomento
{
    public partial class FormUsuario : Form
    {
        ...
        string data_source = "Server=localhost;Database=info06;Uid=root;"; //fazer conexão com o banco de dados
        ...
        private void butSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string sql = "Insert into usuario(nome,cpf,endereço,numero,complemento,bairro,cidade,email,senha,criado_por)" +
                        "VALUES (@nome,@cpf,@endereço,@numero,@complemento,@bairro,@cidade,@email,@senha,@criado_por)";
                    ...
                    using (MySqlCommand comando = new MySqlCommand(sql, conexao))
                    {
                        ...
                        comando.Parameters.AddWithValue("criado_por", butAtualizar);
                    ...
                    txtNome.Clear();
                    butAtualizar.Clear(); //para cria uma variavel la em baixo de LimparCampo();
                }
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                using (conexao = new MySqlConnection(data_source))
                {
                    ...
                    string sql = "SELECT into usuario (nome,cpf,endereço,numero,complemento,bairro,cidade,email,senha)"+
        ...
        private void butAtualizar_Click(object sender, EventArgs e) //para atualizar no caso fazer o update, colocar no botão 
        {
            try
            {
                using (conexao = new MySqlConnection(data_source))
                {
                    string sql = "UPDATE usuarios SET senha = @senha, usuario = @usuario, criado_por = @criadoPor WHERE id = @id";
                    ...
                    using (MySqlCommand comando = new MySqlCommand(sql, conexao))
                    {
                        ...
                        comando.Parameters.AddWithValue("@criadoPor", butAtualizar.Text);
                    ...
                    LimparCampos(); //ser feito uma variavel no campo de cadastro que limpa o nome 
                }
            }
            catch (Exception ex)
            {
